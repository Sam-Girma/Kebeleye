...
import 'package:kebeleyeapp/application/core/report_event.dart';
...
class ReportBloc extends Bloc<ReportEvent, ReportState> {
  ...
  ReportBloc(this.reportRepository) : super(ClearedReportState()) {
    ...
    on<SendReportEvent>((SendReportEvent event, Emitter emit) async {
      emit(ReportingState());
      try {
        final report = await reportRepository.create(Report(
    ...
    on<MemberReportsFetchEvent>(
        (MemberReportsFetchEvent event, Emitter emit) async {
    ...
    on<UpdateReportEvent>((UpdateReportEvent event, Emitter emit) async {
      ...
      try {
        final reports =
            await reportRepository.update(event.postcontent, event.report);
