...
import 'package:kebeleyeapp/application/core/report_state.dart';

import '../../Infrastructure/core/Feedback_repository.dart';
...
class FeedbackBloc extends Bloc<FeedbackEvent, FeedbackState> {
  ...
  FeedbackBloc(this.feedbackRepository) : super(ClearedFeedbackState()) {
    ...
    on<SendFeedbackEvent>((SendFeedbackEvent event, Emitter emit) async {
      ...
      try {
        final report = await feedbackRepository.create(Feedback(
    ...
    on<MembersFeedbackFetchEvent>(
        (MembersFeedbackFetchEvent event, Emitter emit) async {
      emit(FetchingFeedbackState());
      try {
        final feedbacks =
            await feedbackRepository.fetchFeedbackByuser(event.user);
        emit(FetchingFeedbackSuccessful(feedbacks));
      } catch (error) {
        emit(FetchingFeedbackFailed());
      }
    });
    on<ToupdateFeedbackEvent>(
        (ToupdateFeedbackEvent event, Emitter emit) async {
    ...
    on<UpdateFeedbackEvent>((UpdateFeedbackEvent event, Emitter emit) async {
      ...
      try {
        final reports = await feedbackRepository.update(
            event.feedbackcontent, event.feedback);
